...
rbs_smry <- sim_tbls$data_mse %>%
  ...
  group_by(scenario) %>%
  summarize(
    mn_rbs_icv = mean(imp_cv_rbs) / 100,
    mn_rbs_cvi = mean(cv_imp_rbs) / 100
  )
...
std_smry <- sim_tbls$data_mse %>%
  ...
  group_by(scenario) %>%
  summarize(
    mn_std_icv = mean(imp_cv_std) / 100,
    mn_std_cvi = mean(cv_imp_std) / 100
  )
...
rmse_smry <- sim_tbls$data_mse %>%
  ...
  group_by(scenario) %>%
  summarize(
    mn_rmse_icv = mean(imp_cv_rmse) / 100,
    mn_rmse_cvi = mean(cv_imp_rmse) / 100
  )

within(rbs_smry, {
  rbs_s1_icv <<- round(mn_rbs_icv[scenario == 's1'], 3)
  rbs_s2_icv <<- round(mn_rbs_icv[scenario == 's2'], 3)
  rbs_s3_icv <<- round(mn_rbs_icv[scenario == 's3'], 3)
  rbs_s1_cvi <<- round(mn_rbs_cvi[scenario == 's1'], 3)
  rbs_s2_cvi <<- round(mn_rbs_cvi[scenario == 's2'], 3)
  rbs_s3_cvi <<- round(mn_rbs_cvi[scenario == 's3'], 3)
})

within(std_smry, {
  std_s1_icv <<- round(mn_std_icv[scenario == 's1'], 3)
  std_s2_icv <<- round(mn_std_icv[scenario == 's2'], 3)
  std_s3_icv <<- round(mn_std_icv[scenario == 's3'], 3)
  std_s1_cvi <<- round(mn_std_cvi[scenario == 's1'], 3)
  std_s2_cvi <<- round(mn_std_cvi[scenario == 's2'], 3)
  std_s3_cvi <<- round(mn_std_cvi[scenario == 's3'], 3)
})

within(rmse_smry, {
  rmse_s1_icv <<- round(mn_rmse_icv[scenario == 's1'], 4)
  rmse_s2_icv <<- round(mn_rmse_icv[scenario == 's2'], 4)
  rmse_s3_icv <<- round(mn_rmse_icv[scenario == 's3'], 4)
  rmse_s1_cvi <<- round(mn_rmse_cvi[scenario == 's1'], 4)
  rmse_s2_cvi <<- round(mn_rmse_cvi[scenario == 's2'], 4)
  rmse_s3_cvi <<- round(mn_rmse_cvi[scenario == 's3'], 4)
})

```


For scenario 1, the overall bias (\ie aggregating over the 18 combinations of training sample size, predictor to junk ratios, and MD pattern) of $R^2$ estimates using \cvi\space was `r rbs_s1_cvi`, whereas the corresponding overall bias using \icv\space was `r rbs_s1_icv` (\textbf{Table} \ref{tab:bias}). In scenario 2, the bias of \cvi\space was `r rbs_s2_cvi` and the bias of \icv\space was `r rbs_s2_icv`. Similarly, in scenario 3, the biases of \cvi\space and \icv\space were `r rbs_s3_cvi` and `r rbs_s3_icv`, respectively. The overall standard deviation of $R^2$ estimates was higher for \cvi\space versus \icv\space in all three scenarios, but the difference was most pronounced in scenario 2 (`r std_s2_cvi` versus `r std_s2_icv`; \textbf{Table} \ref{tab:variance}). A remarkable result is that, despite its bias, the lower variance of \icv\space compared to \cvi\space in scenario 2 leads to a lower overall root-mean-squared error of external $R^2$ estimates for \icv\space versus \cvi\space (`r rmse_s2_icv` versus `r rmse_s2_cvi`; \textbf{Table} \ref{tab:rmse}).    
