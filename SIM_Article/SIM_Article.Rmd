...
within(rbs_smry, {
  rbs_s1_icv <<- round(mn_rbs_icv[scenario == 's1'], 4)
  rbs_s2_icv <<- round(mn_rbs_icv[scenario == 's2'], 4)
  rbs_s3_icv <<- round(mn_rbs_icv[scenario == 's3'], 4)
  rbs_s1_cvi <<- round(mn_rbs_cvi[scenario == 's1'], 4)
  rbs_s2_cvi <<- round(mn_rbs_cvi[scenario == 's2'], 4)
  rbs_s3_cvi <<- round(mn_rbs_cvi[scenario == 's3'], 4)
})
...
For scenario 1, the overall bias (\ie aggregating over the 18 combinations of training sample size, predictor to junk ratios, and MD pattern) of $R^2$ estimates using \cvi\space was `r rbs_s1_cvi`, whereas the corresponding overall bias using \icv\space was `r rbs_s1_icv` (\textbf{Table} \ref{tab:bias}). In scenario 2, the bias of \cvi\space was `r rbs_s2_cvi` and the bias of \icv\space was `r rbs_s2_icv`. Similarly, in scenario 3, the biases of \cvi\space and \icv\space were `r rbs_s3_cvi` and `r rbs_s3_icv`, respectively. The overall standard deviation of $R^2$ estimates was higher for \cvi\space versus \icv\space in all three scenarios, but the difference was most pronounced in scenario 2 (`r std_s2_cvi` versus `r std_s2_icv`; \textbf{Table} \ref{tab:variance}).
